...
public class A_QSort {
    ...
    int[] getAccessory(InputStream stream) throws FileNotFoundException {
        ...
        // массив для точек и их индексов
        ...
        for (int i = 0; i < m; i++) {
            ...
            while (j < n && segments[j].stop < point) {
                j++; // пропуск отрезков, которые закончились до точки
            }
            int count = 0;
            for (int k = j; k < n; k++) {
                if (segments[k].start <= point && segments[k].stop >= point) {
                    count++; // подсчет отрезков, которые охватывают точку
                } else {
                    break;
                }
            }
            result[indexedPoints[i][1]] = count; // запись результата по индексу точки
        }
