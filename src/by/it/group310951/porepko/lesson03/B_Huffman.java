...
public class B_Huffman {
    ...
    String decode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i <= count; i++) {
            ...
            if (matcher.matches()) {
                char key = matcher.group(1).charAt(0);  // получаем ключ
        ...
        while (encodedString.length() > 0) {
            for (char symbol : codesMap.keySet()) {
                if (encodedString.startsWith(codesMap.get(symbol))) {
                    result.append(symbol);

                    // добавляем символ к ответу и подрезаем начало строки
        ...
        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! КОНЕЦ ЗАДАЧИ !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
        return result.toString(); // строка abacabad
    }
