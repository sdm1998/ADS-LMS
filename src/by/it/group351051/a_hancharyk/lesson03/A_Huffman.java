...
public class A_Huffman {
    ...
    abstract class Node implements Comparable<Node> {
        ...
        //конструктор по умолчанию(функция, которая создает переменную этого типа)
        ...
        //или для сортировок
        //@override - перегрузка метода(функции) В данном случае перегружаем станджартный метод сравнения
        @Override
    ...
    //extends -> наследование от базого(родителя)
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        String s = scanner.next();//считали строку
        ...
        while (priorityQueue.size() > 1) {
            Node left = priorityQueue.poll(); //вынимаем левый узел
            Node right = priorityQueue.poll(); //вынимаем правый узел

            //возвращая этого родителя обратно в очередь
        ...
        StringBuilder sb = new StringBuilder();//строка, которую можно изменять

        char[] res =  s.toCharArray();
        for (int i = 0; i < res.length; i++) {
            sb.append(codes.get(res[i]));//добавляем в итоговую строку код указангго символа
        }
