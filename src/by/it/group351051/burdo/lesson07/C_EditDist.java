...
public class C_EditDist {
    ...
    String getDistanceEdinting(String one, String two) {
        ...
        // задаем матрицу для вычисления действий редактирования
        ...
        for (int i = 1; i < editingMatrix.length; i++) {
            for (int j = 1; j < editingMatrix[i].length; j++) {
                ...
                // возможные операции
                int insert = editingMatrix[i - 1][j] + 1;
                int delete = editingMatrix[i][j - 1] + 1;
                int replace = editingMatrix[i - 1][j - 1] + cost;
